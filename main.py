...
def adjust_learning_rate(optimizer, epoch):     # 每隔step_size个epoch，学习率×0.1
    ...
    for param_group in optimizer.param_groups:     # 动态调整学习率
        param_group['lr'] = lr
        
def l2_penalty(model,model_init,weight_decay):    #l2正则项
    l2_loss = 0
...
def run_epoch(args, model, model_init, train_loader, criterion=torch.nn.CrossEntropyLoss(), optimizer=None, scheduler=None, epoch=0, weight_decay=0.0, mode='train', quiet=False):
    ...
    with torch.set_grad_enabled(mode != 'test'):    # 若为False，则接下来所有节点都不可求导了
        for batch_idx, (data, target) in enumerate(train_loader):
            data, target = data.to(args.device), target.to(args.device)
            
            if args.lossfn=='mse':
                target=(2*target-1)
                target = target.type(torch.cuda.FloatTensor).unsqueeze(1)     # 在第一个维度后重新插入一个维度
...
if __name__ == '__main__':
    ...
    # 获取数据集，get_loaders函数在datasets_multiclass.py函数中
    ...
    torch.save(model.state_dict(), f"checkpoints/{args.name}_init.pt")     # state_dict()存放模型中的权重和偏执系数
    
    parameters = model.parameters()    # 存放模型权重
    if args.unfreeze_start is not None:
        parameters = []
        layer_index = 1e8
