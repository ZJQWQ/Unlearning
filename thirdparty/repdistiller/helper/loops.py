...
def train_negrad(epoch, train_loader, delete_loader, model, criterion, optimizer, alpha, opt, quiet=False):
    ...
    batch_time = AverageMeter()    # val,sum,count,avg
    ...
    for idx, ((input, target), (del_input, del_target)) in enumerate(zip(train_loader, cycle(delete_loader))):
        ...
        data_time.update(time.time() - end)      # 将时间更新到data_time中

        input = input.float()
        del_input = del_input.float()
        if torch.cuda.is_available():     # 转为GPU
            input = input.cuda()
            target = target.cuda()
        ...
        loss = alpha*r_loss - (1-alpha)*del_loss   # 见论文p13，对小规模实验，alpha=0.95，对大规模实验，alpha=0.9999，即要求在retain集上的误差尽量小，在forget集上误差尽量大

        if not quiet:
            acc1, acc5 = accuracy(output, target, topk=(1, 5))   # 得到top1和top5的正确率，并进行更新
        ...
        if not quiet:
            # print info
            if idx % opt.print_freq == 0:     # 每更新 print_freq 次，进行输出
                print('Epoch: [{0}][{1}/{2}]\t'
    ...
    if not quiet:       # 输出最终的top1和top5平均正确率
        print(' * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}'
              .format(top1=top1, top5=top5))

    return top1.avg, losses.avg

# 用train数据集对原模型参数进行 finetuning 
...
# train_distill(epoch, forget_loader, module_list, swa_model, criterion_list, optimizer, args, "maximize")
def train_distill(epoch, train_loader, module_list, swa_model, criterion_list, optimizer, opt, split, quiet=False):
    """One epoch distillation"""
    # set modules as train()
    for module in module_list:    # module_list中为 module_s 和 module_t
        module.train()
    # set teacher as eval()
    module_list[-1].eval()


    criterion_cls = criterion_list[0]    # criterion_cls = nn.CrossEntropyLoss()
    criterion_div = criterion_list[1]    # DistillKL(args.kd_T)
    criterion_kd = criterion_list[2]     # DistillKL(args.kd_T)
    ...
    for idx, data in enumerate(train_loader):
        ...
        # cls + kl div
        loss_cls = criterion_cls(logit_s, target)      # 返回学生模型和真值的分布的 交叉熵损失
        loss_div = criterion_div(logit_s, logit_t)     # 返回学生模型和老师模型的输出分布的 KL 散度值

        # other kd beyond KL divergence
        if opt.distill == 'kd':    # 默认为kd
            loss_kd = 0
        elif opt.distill == 'hint':
            f_s = module_list[1](feat_s[opt.hint_layer])
            f_t = feat_t[opt.hint_layer]
            loss_kd = criterion_kd(f_s, f_t)     # f_s和f_t的 KL 散度值
        ...
        if split == "minimize":   # 以上三类loss相加
            loss = opt.gamma * loss_cls + opt.alpha * loss_div + opt.beta * loss_kd
        elif split == "maximize":   # 只关注 学生模型和老师模型的输出分布的 KL 散度值
            loss = -loss_div
